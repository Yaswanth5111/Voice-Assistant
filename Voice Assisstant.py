...
for voice in voices:
    if "male" in voice.name.lower() and "english" in voice.name.lower() and "united states" in voice.name.lower():
...
def get_daily_news():
    try:
        url = 'https://newsapi.org/v2/top-headlines?country=us&apiKey=967057fb48d3408bbd8a29ce2ab2b11c'
        response = requests.get(url)
        data = response.json()
...
response_label = tk.Label(window, text="Genie: ", font=("Arial", 14), wraplength=400, fg="#00ff00", bg="#1f1f1f")
response_label.pack(pady=20)

# Create a button to start listening
listen_button = tk.Button(window, text="Listen", font=("Arial", 14), command=listen_for_command)
listen_button.pack(pady=20)

# Create a button to stop the assistant
stop_button = tk.Button(window, text="Stop", font=("Arial", 14), bg="#FF0000", fg="#000000", command=window.destroy)
